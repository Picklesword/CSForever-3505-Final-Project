	...
	if(!strcmp(parsed,"cell"))
	{
	  ...
	  else
	  {
	    ...
	    string spreadsheet = users_editing[connFd];
            ...
            if(opened_spreadsheets[spreadsheet]->cells.count(cell_name) != 0) //added the first two lines- Dharani
	    {
	    ...
	    {
	      pair<string, string> oldValue (cell_name, "");//added these two lines to handle undo - Dharani
	      opened_spreadsheets[spreadsheet]->stackOfChanges.push(oldValue);
