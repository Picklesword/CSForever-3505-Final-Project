...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name);                                      // adds the current cell's name to list of visted cells 
            foreach (String n in GetDirectDependents(name))         // iterates through the dependents of current cell
            {
                if (n.Equals(start))                               //  checks to see if the current dependent's name matches the name of the cell from which all the dependents have come from 
                {
                    throw new CircularException();                 // if the names did match it would indicate a circular dependicy so an exception must be thrown 
                }
                else if (!visited.Contains(n))                     // makes sure the dependent has not been visited already 
                {
                    Visit(start, n, visited, changed);              // recursive call now getting all the dependents of the current dependent or the indirect dependents of the orginal. perserves the starting name and the visted and changed lists 
                }
            }
            changed.AddFirst(name);                                // places the current cell's name in the list of changed cells 
        }
